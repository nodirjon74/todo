...
class ToDoTableViewController: UITableViewController, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    var mcAdvertiserAsistant: MCAdvertiserAssistant!
    ...
    func addNewToDo() {
        let addAlert = UIAlertController(title: "New To Do", message: "Enter A Titile", preferredStyle: .alert)
        addAlert.addTextField {(textfield: UITextField) in textfield.placeholder = "To Do Item Title"
    ...
    func showConnectivityAction() {
        ...
        actionSheet.addAction(UIAlertAction(title: "Host Session", style: .default, handler: {(action: UIAlertAction) in
            self.mcAdvertiserAsistant = MCAdvertiserAssistant(serviceType: "ba-td", discoveryInfo: nil, session: self.mcSession)
            self.mcAdvertiserAsistant.start()
        }))
    ...
    func completeTodoItems(_ indexPath: IndexPath) {
        ...
        if let cell = tableView.cellForRow(at: indexPath) as? ToDoTableViewCell {
            ...
            UIView.animate(withDuration: 0.1, animations: {cell.transform = cell.transform.scaledBy(x: 1.5, y: 1.5)}, completion: {(success) in
                UIView.animate(withDuration: 0.3, delay: 0, options: .curveEaseOut, animations: {cell.transform = CGAffineTransform.identity}, completion: nil)
            })
        }
        ...
        if mcSession.connectedPeers.count > 0 {
            if let todoData = DataManager.loadData(todoItem.itemIdentifier.uuidString){
                do {
                    ...
                    fatalError("Could not send to do item")
                }
            }else{
    ...
    func strikeThroughText(_ text: String) -> NSAttributedString {
        ...
        attributedString.addAttribute(NSMutableAttributedString.Key.strikethroughStyle, value: 1, range: NSMakeRange(0, attributedString.length))
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let shareAction = UITableViewRowAction(style: .normal, title: "Share", handler: {(action: UITableViewRowAction, indexPath: IndexPath) in
            let todoItem = self.todoItems[indexPath.row]
            self.sendTodo(todoItem)
        })
        shareAction.backgroundColor = UIColor(named: "mainGreenColor")
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        do {
            ...
            fatalError("Unable to precess the resiver data")
        }
    }
